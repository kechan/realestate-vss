...
# multiprocessing/threading stuff
...
PERFORM_PREPROCESSING_IN_DATALOADER = False
...
class RetrievalDataset(IterableDataset):
  ...
  def __iter__(self):
    while True:
      ...
      # paths_digest = hashlib.md5("".join(sorted(paths)).encode()).hexdigest()    # set hash digest
...
def main():
  ...
  dataset_args = {
    "teacher_embeddings": train_teacher_image_embeddings,
    "student0_embeddings": train_student0_image_embeddings,
    "image_names": train_image_names,
    "teacher_index": train_index,
    "batch_size": batch_size,
    "pos_k": pos_k,
    "neg_unif": neg_unif,
  }
  if PERFORM_PREPROCESSING_IN_DATALOADER:
    dataset_args.update({
      "preprocess": student_preprocess,
      "data_dir": photos_dir,
    })

  dataset = RetrievalDataset(**dataset_args)
  ...
  for step, batch in enumerate(dataloader, 1):
    ...
    # A) Forward pass (may include preprocessing)
    ...
    print(f" Model forward pass took {(t1 - t0):.2f}s")
  ...
  print(f"▶ Total time for {num_steps} steps: {total_time:.2f}s "
      f"(avg {total_time/num_steps:.2f}s per step)")
      
  with open(chkpt_dir/f"student_model_chkpt_set_{image_set_id}_epoch_1_logs.json", "w") as f:
    json.dump(logs, f, indent=4)

if __name__ == '__main__':
  # on macOS with spawn you should also call freeze_support()
  mp.freeze_support()
  # force spawn as before
  try:
      mp.set_start_method('spawn', force=True)
  except RuntimeError:
      pass
